...
class TabReclip_VM(TabReclip_View):
    def __init__(self, parent=None):
        ...
        self.pushButton_print_score.clicked.connect(self.printing_perview)
    ...
    def get_final_images(self) -> list[ImageArray] | None:
        ...
        if path == []:
            log.warning(f"No {working_dir.name}*[0-9]* image found")
            return None
    ...
    def printing_perview(self) -> None:
    ...
    def show_style(self) -> None:
        ...
        if not (working_dir / filename).exists():
            log.warning(
                f"Recliped image not found:{working_dir / filename}, do reclip first"
            )
            return
    ...
    def load_style(self, path: JsonPath) -> None:
        if not path.exists():
            log.warning(f"Style reset does not exist: {path}")
            return
